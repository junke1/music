...
// 电台详情
const djDetail = ({ rid = '' }) => {
  return api.get(`/dj/detail?rid=${rid}`, {});
};
// 电台节目
const djProgram = ({ rid = '', limit = 30, offset = 0, asc = false }) => {
  return api.get(
    `/dj/program?rid=${rid}&limit=${limit}&offset=${offset}&asc=${asc}`
  );
};
...
// 最近播放-歌曲
const recentSong = ({ limit = 100 }) => {
  return api.get(`/record/recent/song?limit=${limit}`, {});
};
